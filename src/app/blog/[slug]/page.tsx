...
import BlogPostSidebar, { getRecentPosts } from '../../components/BlogPostSidebar'
import {
  buildPageMetadata,
      ...
      <article>
        ...
        <section className="relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] flex w-screen -mt-16 min-h-[420px] items-center justify-center overflow-hidden bg-slate-950 md:-mt-20 md:min-h-[520px] lg:min-h-[580px]">
          ...
          <div className="relative z-10 mx-auto w-full max-w-4xl px-6 pb-16 pt-24 text-center md:pb-20 md:pt-28">
            ...
            <p className="mt-6 text-sm font-medium text-white/85 md:mt-8 md:text-base">{post.date}</p>
          </div>
        </section>

        {/* Nerox-style body: content + sidebar */}
        <section className="pb-16 pt-10 md:pb-24 md:pt-14">
          <div className="container mx-auto px-4 lg:px-8">
            ...
            >
              <Link href="/" className="whitespace-nowrap hover:text-slate-900 dark:hover:text-white">
                {t(locale, 'blog.breadcrumb.home')}
              </Link>
              <span aria-hidden>/</span>
              <Link href="/blog" className="whitespace-nowrap hover:text-slate-900 dark:hover:text-white">
            ...
            <div className="grid gap-12 lg:grid-cols-12 lg:gap-10 xl:gap-14">
              <div className="min-w-0 lg:col-span-8">
                ...
                <p className="mb-10 text-base leading-relaxed text-slate-700 dark:text-slate-300 md:text-lg">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {post.tags.map((tag) => (
                        <Link
                          key={tag}
                          href="/blog"
                          className="inline-flex border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:border-[var(--apsod-accent)] hover:text-[var(--apsod-accent)] dark:border-slate-700 dark:text-slate-300"
                        >
                          {tag}
                        </Link>
                      ))}
                    ...
                    <div className="grid gap-8 sm:grid-cols-2">
                      {recentPosts.map((item) => (
                        <article key={item.slug}>
                          <Link
                            href={`/blog/${item.slug}`}
                            className="group mb-5 block overflow-hidden bg-slate-100 dark:bg-slate-900"
                          >
                          ...
                          <h3 className="font-display mb-2 text-lg font-bold leading-snug tracking-tight text-slate-900 dark:text-white">
                            <Link href={`/blog/${item.slug}`} className="hover:text-[var(--apsod-accent)]">
                              {item.title}
                            </Link>
                          </h3>
                ...
                <BlogShare url={articleUrl} title={post.title} locale={isEn ? 'en' : 'ru'} />
