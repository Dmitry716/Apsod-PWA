  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <section className="pb-8">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-slate-200 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 overflow-hidden">
            {contactInfo.map((item) => (
              <div
                key={item.title}
                className="bg-white dark:bg-gray-950 p-6"
              >
                ...
                {item.link ? (
                  <a
                    ...
                    className="text-slate-800 dark:text-slate-100 hover:underline font-medium text-sm"
                  >
                    {item.value}
                  </a>
                ) : (
                  <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">{item.value}</p>
                )}
      ...
      <section className="py-10">
        <div className="container mx-auto px-4 max-w-3xl">
          <Suspense
            fallback={
              <div className="border border-slate-200 dark:border-slate-700 p-10 text-center text-slate-500">
                Загрузка формы…
              </div>
            }
          >
          ...
          <div className="flex flex-wrap gap-3">
            {socialLinks.map((social) => (
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 hover:border-slate-900 dark:hover:border-white transition-colors text-sm font-medium"
